...
function log(message) {
  if (global.firebaseLogger) {
    global.firebaseLogger.log(message);
  }
}
...
function validateRequest(authToken, twilioHeader, url, params) {
  ...
  log(`validateRequest signatureWithoutPort=${signatureWithoutPort} validSignatureWithoutPort=${validSignatureWithoutPort}`);
...
function validateRequestWithBody(authToken, twilioHeader, url, body) {
  log(`validateRequestWithBody ${url}`);
...
function validateExpressRequest(request, authToken, opts) {
  ...
  log(`validateExpressRequest ${webhookUrl}`);
  ...
  return function hook(request, response, next) {
    ...
    if (opts.validate) {
      ...
      if (!request.header('X-Twilio-Signature')) {
        log('No signature header error - X-Twilio-Signature header does not exist, maybe this request is not coming from Twilio.');
      ...
      if (!opts.authToken) {
        log('[Twilio]: Error - Twilio auth token is required for webhook request validation.');
        ...
        if (valid) {
          ...
          log('Twilio Request Validation Failed.');
          return response
            .type('text/plain')
            .status(403)
